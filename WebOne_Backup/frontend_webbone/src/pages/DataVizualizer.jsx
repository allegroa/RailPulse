...
function DataVisualizer() {
  ...
  const loadTolerances = async (folderName) => {
    if (!folderName) return;
    try {
      const cacheBuster = Date.now();
      const resp = await axios.get(`/api/files/tolerances?folder=${encodeURIComponent(folderName)}&_cb=${cacheBuster}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (resp.data) {
        setTolerances(resp.data);
      } else {
        setTolerances({});
      }
    } catch (e) {
  ...
  const saveTolerances = async (folderName, nextTolerances) => {
    if (!folderName) return;
    try {
      await axios.post('/api/files/tolerances/save', {
        folder: folderName,
        tolerances: nextTolerances
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (token) loadTolerances(folderParam);
  }, [searchParams, token]);
  ...
  const updateTolerance = (col, val) => {
    setTolerances(prev => {
      ...
      saveTolerances(folderParam, next);
      return next;
    });
  };
  ...
  const fetchFiles = async () => {
    try {
      const folderParam = searchParams.get('folder') || 'upload';
      const res = await axios.get(`/api/files/${encodeURIComponent(folderParam)}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const loadServerCsv = async (fileName, folderName = 'upload', requestedSampleSize = sampleSize, storeFull = false) => {
    ...
    try {
      ...
      const dbResp = await axios.get(dbUrl, { headers: { Authorization: `Bearer ${token}` } });
      if (Array.isArray(dbResp.data)) {
    ...
    await loadTolerances(folderName);
  ...
  if (!storeFull && useSampling) {
    (async () => {
      ...
      const ss = requestedSampleSize || sampleSize;
      if (totalDataRows <= ss) {
        // small file — fallback to full local load via normal path below
      } else {
        const detectedLower = (hdrs || []).map(h => (h || '').toLowerCase());
        const hasKm = detectedLower.includes('km');
        const useOrdered = hasKm || !!selectedX;
        ...
        // otherwise fall through to single-pass reservoir/full parser below
      }
    })();
    return;
  }
  ...
  const uploadToServer = async (file) => {
    ...
    try {
      await axios.post(`/api/files/${encodeURIComponent('upload')}/upload`, formData, { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } });
      await fetchFiles();
  ...
  const saveSingularities = async () => {
    ...
    try {
      await axios.post('/api/files/singularities/save', {
        folder: folderParam,
        file: fileParam,
        singularities
      }, {
  ...
  return (
    <div className="w-full py-2 px-1">
      {parseError && <div className="mb-4 bg-red-50 border-l-4 border-red-400 p-3 rounded"><div className="text-sm text-red-700">{parseError}</div></div>}
      {uploadStatus && <div className={`mb-4 ${uploadStatus.type === 'success' ? 'bg-green-50 border-l-4 border-green-400 text-green-700' : 'bg-red-50 border-l-4 border-red-400 text-red-700'} p-3 rounded`}><div className="text-sm">{uploadStatus.msg}</div></div>}
        ...
        <div className="flex justify-between items-center w-full">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 flex items-center">
              ...
              {t('appTitle') || 'Data Visualizer'} <span className="text-sm font-normal text-blue-600 bg-blue-100 px-2 py-1 rounded ml-2">v1.6</span>
            </h2>
            <p className="text-slate-500 mt-1">{t('visualizeExplore') || 'Visualize and explore your CSV data'}</p>
          </div>
          <div className="flex gap-4 items-center">
            <select value={i18n.language} onChange={e => i18n.changeLanguage(e.target.value)} className="border-slate-300 rounded text-sm px-2 py-1">
              <option value="it">Italiano</option><option value="en">English</option><option value="zh">中文</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="flex flex-col gap-6">
          {infoPairs && infoPairs.length > 0 && (
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="border-b border-slate-200 bg-slate-50 px-6 py-4 flex items-center justify-between">
                <h3 className="text-lg font-medium text-slate-700">{t('infoTitle') || 'Info'}</h3>
                <div className="text-sm text-slate-500">{t('metadataExtracted') || 'Metadata extracted from upload'}</div>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="border-b border-slate-200 bg-slate-50 px-6 py-4 flex items-center justify-between">
              <h3 className="text-lg font-medium text-slate-700">{t('configTitle') || 'Data / Chart Configuration'}</h3>
              <div className="flex items-center gap-3">
                <label className="inline-flex items-center px-3 py-2 border border-transparent rounded-lg text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 cursor-pointer">
                  ...
                  <input type="file" accept=".csv" className="hidden" onChange={e => { if (e.target.files[0]) handleLocalFile(e.target.files[0]); }} />
                </label>
                <label className="inline-flex items-center px-3 py-2 border border-transparent rounded-lg text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 cursor-pointer">
                  Upload to server
                  <input type="file" accept=".csv" className="hidden" onChange={e => { const f = e.target.files[0]; if (f) { setUploadCandidate(f); setShowUploadModal(true); } }} />
                </label>
                {parseProgress && !parseProgress.done && (
                  <div className="inline-flex items-center px-3 py-2 bg-white border border-slate-200 rounded text-sm text-slate-600">
                      <svg className="w-4 h-4 mr-2 animate-spin text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                      </svg>
                      Parsing {parseProgress.parsed || 0} rows{parseProgress.seen ? ` (seen ${parseProgress.seen})` : ''}... {isFullLoad ? <span className="text-xs text-amber-600 ml-2">full load</span> : null}
                    </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 grid grid-cols-1 gap-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">{t('loadServer') || 'Server files (upload/)'}</label>
                <div className="border border-slate-200 rounded-md p-2 max-h-48 overflow-auto">
                  <>
          <div className="flex font-medium text-sm text-slate-600 py-1 px-2 border-b border-slate-200">
          ...
          {availableFiles.length === 0 ? (
                    <div className="text-sm text-slate-500">{t('noFileSelected') || 'No files'}</div>
                  ) : (
                    availableFiles.map(f => {
                      ...
                      return (
                        <div key={name} className="flex flex-col sm:flex-row sm:items-center justify-between py-1.5 px-2 hover:bg-slate-50 rounded border-b border-slate-100 last:border-0 group">
                          <button className="text-sm text-slate-700 text-left font-medium truncate flex-1" onClick={() => loadServerCsv(name, searchParams.get('folder') || 'upload')}>{name}</button>
                          <div className="flex items-center gap-4 text-xs text-slate-400 mt-1 sm:mt-0 opacity-80 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                            {date && <span>{date}</span>}
                            {size && <span className="bg-slate-100 px-1.5 py-0.5 rounded mr-2">{size}</span>}
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden min-h-[300px] flex flex-col">
          <div className="border-b border-slate-200 bg-slate-50 px-6 py-4 flex items-center justify-between">
            <h3 className="text-lg font-medium text-slate-700">{t('mapTitle') || 'Google Maps'}</h3>
          </div>
      ...
      {headers.length > 0 ? (
        <>
          <div className="relative bg-white rounded-xl shadow-md overflow-hidden mb-6 p-6" style={{ height: Math.max(420, selectedYs.length * 150) + 100 }}>
            <div className="absolute top-4 right-4 z-40 flex items-center gap-2">
              {geoDatasets.length > 1 && (
                <button className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded shadow text-sm flex items-center font-medium border border-slate-300" onClick={() => setShowGeoModal(true)}>Gestione Acquisizioni</button>
              )}
              <button className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded shadow text-sm flex items-center font-medium border border-slate-300" onClick={() => chartRef.current?.resetZoom()}>{t('resetZoom') || 'Reset Zoom'}</button>
              <button className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded shadow text-sm flex items-center font-medium border border-slate-300" onClick={saveSingularities}>{t('saveDatabase') || 'Salva DB Linee'}</button>
              <div className="relative">
                <button className="bg-slate-100 hover:bg-slate-200 text-slate-700 w-8 h-8 flex items-center justify-center rounded shadow font-bold border border-slate-300" onClick={() => setShowYMenu(!showYMenu)}>⋯</button>
                {showYMenu && (
                  <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-md shadow-lg z-50">
                    <div className="px-4 py-2 border-b font-medium text-sm text-slate-700 flex justify-between items-center">{t('configureGraphs') || 'Configura Grafici'} (Asse Y) <button onClick={() => setShowYMenu(false)} className="text-slate-400 hover:text-slate-700">&times;</button></div>
            ...
            {contextMenu && (
              <div className="absolute bg-white rounded-lg shadow-xl border border-slate-200 z-50 overflow-hidden" style={{ left: contextMenu.x + 20, top: Math.max(contextMenu.y - 50, 10) }}>
                <div className="bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600 border-b border-slate-200 flex justify-between items-center">
                  <span>Km: {contextMenu.formattedKm || contextMenu.kmLabel}</span><button onClick={() => setContextMenu(null)} className="ml-4 text-slate-400 hover:text-slate-700">&times;</button>
                </div>
                <ul className="py-1">
                  {[ { type: 'Semaforo', icon: '🚦' }, { type: 'Passaggio a livello', icon: '🚧' }, { type: 'Fabbricato viaggiatori', icon: '🚉' }, { type: 'Scambio', icon: '🛤️' }, { type: 'Cippo', icon: '📍' } ].map(item => (
                    <li key={item.type}>
                      <button className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-blue-50 flex items-center gap-2" onClick={() => { 
                        setSingularities(prev => {
                          const next = [...prev, { km: contextMenu.kmLabel, type: item.type, icon: item.icon }];
                          const fileParam = searchParams.get('file') || lastServerFile?.file;
                          const folderParam = searchParams.get('folder') || lastServerFile?.folder || 'upload';
                          if (fileParam) {
                            axios.post('/api/files/singularities/save', {
                              folder: folderParam,
                              file: fileParam,
                              singularities: next
                            }, { headers: { Authorization: `Bearer ${token}` } }).catch(e => console.error('auto-save error', e));
                          }
                          return next;
                        }); 
                        setContextMenu(null); 

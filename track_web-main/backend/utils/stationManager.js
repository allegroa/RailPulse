...
function getStationFilePath(basePath) {
  const normPath = path.normalize(basePath);
  const tgmSuffix = path.join('DATABASE', 'TGM');
  const dbSuffix = 'DATABASE';
  
  if (normPath.toUpperCase().includes(tgmSuffix.toUpperCase())) {
    const idx = normPath.toUpperCase().indexOf(tgmSuffix.toUpperCase());
    const dbRoot = basePath.substring(0, idx + dbSuffix.length);
    return path.join(dbRoot, 'station.json');
  }

...
import { existsSync, createReadStream, readFileSync, writeFileSync, readdirSync } from 'fs';
...
import { getSessions, getSessionData, getSessionExceedances, getSessionTQI } from './controllers/tgmController.js';
import { addStation, getStations } from './utils/stationManager.js';
import { getMaintenanceRecords, addMaintenanceRecord, updateMaintenanceRecord, deleteMaintenanceRecord } from './utils/maintenanceManager.js';
import { checkNewEmails } from './utils/emailService.js';
import { extractArchive } from './utils/extractor.js';
import { parseCSVFile, extractLineNameData } from './utils/tgmParser.js';
...
// Helper per i percorsi di configurazione
const CONFIG_DIR = path.resolve('configuration');
...
router.post('/configuration', async (req, res) => {
  ...
  try {
    ...
    try {
      const { setupEmailPolling } = await import('./server.js');
      setupEmailPolling();
    } catch (err) {
...
// -------------------------------------------------------------
const TMP_UPLOAD_DIR = path.resolve('tmp_uploads', 'email_queue');
...
router.post('/tgm/sessions/import', upload.any(), async (req, res) => {
  ...
  const tempUploadDir = path.resolve('tmp_uploads', tempDirName);
  const emailQueueDir = path.resolve('tmp_uploads', 'email_queue');
  ...
  try {
    ...
    try {
      ...
      if (lineData && lineData.stazionePartenza) {
        await addStation(Easton, lineData.stazionePartenza);
      }
    } catch (stationErr) {
...
router.get('/tgm/stations', async (req, res) => {
  const targetPath = req.query.path;
  if (!targetPath) {
    return res.status(400).json({ error: 'Path parameter is required' });
  }

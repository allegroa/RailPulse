...
const { parseSessionFolderName, findSessionFiles, parseCSVFile, extractLineNameData } = require('./tgmParser');

/**
 * Ottiene la lista di tutte le sessioni TGM disponibili nella cartella del database
 */
async function getSessions(databasePath) {
  try {
 ...
 */
async function getSessionData(databasePath, sessionId) {
  try {
 ...
 */
async function getSessionExceedances(databasePath, sessionId) {
  try {
    const sessionFolderPath = path.join(databasePath, sessionId);
    const sessionFiles = await findSessionFiles(sessionFolderPath);
 ...
 */
async function getSessionTQI(databasePath, sessionId) {
  try {
    const sessionFolderPath = path.join(databasePath, sessionId);
    const sessionFiles = await findSessionFiles(sessionFolderPath);

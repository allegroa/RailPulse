...
const TRANSLATIONS = {
  it: {
    ...
    tableActions: "Azioni",
    uploadFiles: "Carica Documenti/Foto",
    dropFilesHere: "Trascina i file qui o clicca per selezionare",
  ...
  en: {
    ...
    uploadFiles: "Upload Documents/Photos",
    dropFilesHere: "Drag files here or click to select",
    uploading: "Uploading...",
    viewFile: "View",
    simTitle: "Graphical Integration Simulator",
...
export default function App() {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/${selectedRecord.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(bodyData)
      });
  ...
  const startEditMode = () => {
    setFormData({
      ...
      statusDate: selectedRecord.statusDate || '',
  ...
  const handleDeleteRecord = async (id) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast(data.error || 'Delete error', true);
      }
    } catch (err) {
  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      ...
      if (res.ok && data.files) {
        const newAttachments = [...(formData.attachments || []), ...data.files];
        setFormData(prev => ({ ...prev, attachments: newAttachments }));
        showToast(t('uploadFiles') + ' OK');
      } else {
                      ...
                      <tr key={record.id} onClick={() => setSelectedRecord(record)}>
                        ...
                        <td onClick={(e) => e.stopPropagation()}>
                          <button 
                            ...
                            style={{ padding: '4px 8px', fontSize: '0.75rem', marginLeft: 'auto', display: 'block' }}
                            onClick={() => { setSelectedRecord(record); setTimeout(startEditMode, 0); }}
                          >
                            Edit
                          </button>
                        </td>
          ...
          <div className="modal-content glass-panel">
            ...
            <form onSubmit={handleCreateSubmit} className="modal-form">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  <label>{t('statusDate')}</label>
                  <input required type="date" className="input-field" value={formData.statusDate} onChange={e => setFormData({ ...formData, statusDate: e.target.value })} />
                </div>
              </div>
              ...
              <form onSubmit={handleEditSubmit} className="modal-form">
                ...
                <div className="form-group">
                  ...
                  <div 
                    ...
                    style={{ border: '2px dashed #cbd5e1', borderRadius: '8px', padding: '20px', textAlign: 'center', cursor: 'pointer', background: '#f8fafc', color: 'var(--text-secondary)', transition: 'background 0.2s' }}
                  >
                    <input 
                      type="file" 
                      multiple 
                  ...
                  {formData.attachments && formData.attachments.length > 0 && (
                    <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                      {formData.attachments.map((att, idx) => {
                        ...
                        const baseUrl = API_BASE.replace('/api/maintenance', '');
                        return (
                          <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f1f5f9', padding: '6px 10px', borderRadius: '4px', fontSize: '0.8rem', border: '1px solid #e2e8f0' }}>
                            <span style={{ fontWeight: 500 }}>{fileName}</span>
                            <a href={`${baseUrl}${att}`} target="_blank" rel="noopener noreferrer" style={{ color: '#0ea5e9', textDecoration: 'none', fontWeight: 600 }}>{t('viewFile')}</a>
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="details-grid">
                  ...
                  <div className="details-block">
                    ...
                    <span className="details-value">
                      ...
                      {selectedRecord.statusDate && (
                        <span style={{ display: 'block', marginTop: '4px', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                          il {selectedRecord.statusDate}
                        </span>
                      )}
                    </span>
                  </div>
